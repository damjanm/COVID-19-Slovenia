...
####data
if(exists('limit_date')) dd <- dd[as.Date(substr(dd$datum, 1,7), format = "%d. %m.")<=limit_date+forecast,] # limit data to a given date
...
hospitalizirani<- as.numeric(c(dd$hospitalizirani..trenutno.[-1], dd_forecast$hospitalizirani..trenutno.)) #cumulative! prispevek je lahko negativen!
...
vector[N2-N] logLikicuout;
vector[N2-N] logLikicuin;
vector[N2-N] logLikicu;
vector[N2-N] logLikhospout;
vector[N2-N] logLikhospin;
vector[N2-N] logLikhosp;
vector[N2-N] logLikdeathsh;
int<lower=0> h;
{
  ...
  for (i in (N+1):N2){
    h=h+1;
    logLikcases[h]=neg_binomial_2_lpmf(cases[i,1]|E_cases[i, 1], phi_c);
    logLikdeathsc[h]=neg_binomial_2_lpmf(deathsc[i,1]|E_deathsc[i, 1], phi_dc);
    logLikicuout[h]=neg_binomial_2_lpmf(icuout[i, 1]|E_icuso[i, 1], phi_io);
    logLikicuin[h]=neg_binomial_2_lpmf(icuin[i, 1]|E_icusi[i, 1], phi_ii);
    logLikicu[h]=neg_binomial_2_lpmf(icu[i, 1]|E_icus[i, 1], phi_i);
    logLikhospout[h]=neg_binomial_2_lpmf(hospout[i, 1]|E_hospitalso[i, 1], phi_ho);
    logLikhospin[h]=neg_binomial_2_lpmf(hospin[i, 1]|E_hospitalsi[i, 1], phi_hi);
    logLikhosp[h]=neg_binomial_2_lpmf(hosp[i, 1]|E_hospitals[i, 1], phi_h);
    logLikdeathsh[h]=neg_binomial_2_lpmf(deathsh[i, 1]|E_deathsh[i, 1], phi_dh);
