...
tau1<-0.1
tau2<-0.02*0.3
tau3<-0.07*c(0.43,0.28,0.13,0.16) #assuming data are for Ljubljana, MB, Golnik, Celje, ok??
tau5<-0.2
tau7<-0.2
tau6<-1
tau4<-1
...
model_input <- "combined_hospitals" #ortion of subjects which move from truly infected to icu
...
####data
if(exists('limit_date')){
  dd <- dd[as.Date(substr(dd$datum, 1,7), format = "%d. %m.")<=limit_date+forecast,] # limit data to a given date
  dd_hosp <- dd_hosp[as.Date(substr(dd_hosp$datum, 1,7), format = "%d. %m.")<=limit_date+forecast,] # limit data to a given date
} 

# 
if(out_of_time_fixed) dd_hosp <- dd_hosp[1:(nrow(dd_hosp)-no_of_days),]
if(out_of_time_var) dd_hosp<-dd_hosp[1:(nrow(dd_hosp)-forecast),]

# Take into account forecast:
# dd_hosp_tmp <- dd_hosp[1:forecast,]
# dd_hosp_tmp[,] <- -1
# dd_hosp <- rbind(dd_hosp, dd_hosp_tmp)
...
for(i in 1:ncol(hosp.deathsh)) hosp.deathsh[,i] <- c(0,diff(hosp.deathsh[,i]))
# hosp.deathsh[(nrow(hosp.deathsh)-forecast+1):nrow(hosp.deathsh),] <- -1

####
dd_forecast <- dd[(nrow(dd)-forecast+1):nrow(dd),]
dd <- dd[1:(nrow(dd)-forecast),]
...
hospitalizirani<-as.numeric(c(dd$hospitalizirani..trenutno.[-1], dd_forecast$hos)) #cumulative! prispevek je lahko negativen!
...
hospitaliziraniin<-c(dd$novi[-1], dd_forecast$novi)
...
hospitaliziraniout<-as.numeric(c(dd$iz.bol..oskrbe..vsi.[-1], dd_forecast$iz.bol..oskrbe..vsi.))
...
cases<- c(rep(0,50),as.numeric(c(dd$potrjeni.danes[-1], dd_forecast$potrjeni.danes)))
cases[is.na(cases)]<-0


deaths<- as.numeric(c(dd$umrli..vsi.[-1], dd_forecast$umrli..vsi.)) #cumulative!
...
icu<-c(rep(0,50), as.numeric(c(dd$intenzivna.enota..trenutno.[-1], dd_forecast$intenzivna.enota..trenutno.))) #cumulative! prispevek je lahko negativen!
icu[is.na(icu)] <- 0




icuin<-c(rep(0,50), as.numeric(c(dd$I.i[-1], dd_forecast$I.i))) #cumulative! prispevek je lahko negativen!
icuin[is.na(icuin)] <- 0

icuout<-c(rep(0,50), as.numeric(c(dd$I.o[-1], dd_forecast$I.o))) #cumulative! prispevek je lahko negativen!
icuout[is.na(icuout)] <- 0



deathsc<-c(rep(0,50), as.numeric(c(dd$D.u[-1], dd_forecast$D.u))) #cumulative! prispevek je lahko negativen!
deathsc[is.na(deathsc)] <- 0

deathsh<-c(rep(0,50), as.numeric(c(dd$H.D[-1], dd_forecast$H.D))) #cumulative! prispevek je lahko negativen!
...
d1=data.frame(DateRep=day,Cases=cases[1:length(day)],Deaths=deaths[1:length(day)],Deathsh=deathsh[1:length(day)],Deathsc=deathsc[1:length(day)],Hosp=hospitalizirani[1:length(day)],Icu=icu[1:length(day)],Hospin=hospitaliziraniin[1:length(day)],Hospout=hospitaliziraniout[1:length(day)],Icuin=icuin[1:length(day)],Icuout=icuout[1:length(day)])
d1$date<-d1$DateRep
...
stan_data = list(M=4,N=NULL,deaths=NULL,deathsh=NULL,deathsc=NULL,hosp=NULL,icu=NULL,hospin=NULL,hospout=NULL,icuin=NULL,icuout=NULL,f1=NULL,f2=NULL,f3=NULL,f4=NULL,f5=NULL,f6=NULL,f7=NULL,
...
ff3<-matrix(NA,ncol=4,nrow=N2)
for (m in 1:4){
convolution.3 = function(u) (tau3[m] * ecdf.saved.3(u))
...
obj_len <- length(cases)
reported_cases[[Country]] = as.vector(as.numeric(d1$Cases))
cases=c(as.vector(as.numeric(c(d1$Cases,
                               cases[(obj_len-forecast+1):obj_len])))#,rep(-1,forecast)
        )


deaths=c(as.vector(as.numeric(c(d1$Deaths,
                                deaths[(obj_len-forecast+1):obj_len]
                                )))#,rep(-1,forecast)
         )
deaths_by_country[[Country]] = as.vector(as.numeric(d1$Deaths))


deathsh=c(as.vector(as.numeric(c(d1$Deathsh,
                                 deathsh[(obj_len-forecast+1):obj_len]))) #,rep(-1,forecast)
          )
deathsh_by_country[[Country]] = as.vector(as.numeric(d1$Deathsh))

deathsc=c(as.vector(as.numeric(c(d1$Deathsc,
                                 deathsc[(obj_len-forecast+1):obj_len])))#,rep(-1,forecast)
          )
deathsc_by_country[[Country]] = as.vector(as.numeric(d1$Deathsc))


hosps=c(as.vector(as.numeric(c(d1$Hosp,
                               hospitalizirani[(obj_len-forecast+1):obj_len])))#,rep(-1,forecast)
        )
hosps_by_country[[Country]] = as.vector(as.numeric(d1$Hosp))

hospsin=c(as.vector(as.numeric(c(d1$Hospin,
                                 hospitaliziraniin[(obj_len-forecast+1):obj_len])))#,rep(-1,forecast)
          )
hospsin_by_country[[Country]] = as.vector(as.numeric(d1$Hospin))

hospsout=c(as.vector(as.numeric(c(d1$Hospout,
                                  hospitaliziraniout[(obj_len-forecast+1):obj_len])))#,rep(-1,forecast)
           )
hospsout_by_country[[Country]] = as.vector(as.numeric(d1$Hospout))


icus=c(as.vector(as.numeric(c(d1$Icu,
                              icu[(obj_len-forecast+1):obj_len])))#,rep(-1,forecast)
       )
icus_by_country[[Country]] = as.vector(as.numeric(d1$Icu))

icusin=c(as.vector(as.numeric(c(d1$Icuin,
                                icuin[(obj_len-forecast+1):obj_len])))#,rep(-1,forecast)
         )
icusin_by_country[[Country]] = as.vector(as.numeric(d1$Icuin))

icusout=c(as.vector(as.numeric(c(d1$Icuout,
                                 icuout[(obj_len-forecast+1):obj_len])))#,rep(-1,forecast)
          )
...
####
# # Merge Golnik and Celje:
# hosp.hospitalizirani <- cbind(hosp.hospitalizirani[,1],
#                               hosp.hospitalizirani[,2],
#                               rowSums(hosp.hospitalizirani[,c(3,4)]))
# hosp.hospitalizirani[(N2-forecast+1):N2,3] <- -1
# 
# hosp.hospitaliziraniin <- cbind(hosp.hospitaliziraniin[,1],
#                                 hosp.hospitaliziraniin[,2],
#                                 rowSums(hosp.hospitaliziraniin[,c(3,4)]))
# hosp.hospitaliziraniin[(N2-forecast+1):N2,3] <- -1 
# 
# hosp.hospitaliziraniout <- cbind(hosp.hospitaliziraniout[,1],
#                                  hosp.hospitaliziraniout[,2],
#                                  rowSums(hosp.hospitaliziraniout[,c(3,4)]))
# hosp.hospitaliziraniout[(N2-forecast+1):N2,3] <- -1
# 
# hosp.icu <- cbind(hosp.icu[,1],
#                   hosp.icu[,2],
#                   rowSums(hosp.icu[,c(3,4)]))
# hosp.icu[(N2-forecast+1):N2,3] <- -1
# 
# hosp.icuin <- cbind(hosp.icuin[,1],
#                     hosp.icuin[,2],
#                     rowSums(hosp.icuin[,c(3,4)]))
# hosp.icuin[(N2-forecast+1):N2,3] <- -1 
# 
# hosp.icuout <- cbind(hosp.icuout[,1],
#                      hosp.icuout[,2],
#                      rowSums(hosp.icuout[,c(3,4)]))
# hosp.icuout[(N2-forecast+1):N2,3] <- -1
# 
# hosp.deathsh <- cbind(hosp.deathsh[,1],
#                       hosp.deathsh[,2],
#                       rowSums(hosp.deathsh[,c(3,4)]))
# hosp.deathsh[(N2-forecast+1):N2,3] <- -1


# Add data for other hospitals:

stan_data$hosp = hosp.hospitalizirani
stan_data$hospin = hosp.hospitaliziraniin
stan_data$hospout = hosp.hospitaliziraniout
stan_data$deathsh = hosp.deathsh

stan_data$icu = hosp.icu
stan_data$icuin = hosp.icuin
stan_data$icuout = hosp.icuout

# stan_data$hosp = cbind(stan_data$hosp,hosp.hospitalizirani)
# stan_data$hospin = cbind(stan_data$hospin,hosp.hospitaliziraniin)
# stan_data$hospout = cbind(stan_data$hospout,hosp.hospitaliziraniout)
# stan_data$deathsh = cbind(stan_data$deathsh,hosp.deathsh)
# 
# stan_data$icu = cbind(stan_data$icu,hosp.icu)
# stan_data$icuin = cbind(stan_data$icuin,hosp.icuin)
# stan_data$icuout = cbind(stan_data$icuout,hosp.icuout)
...
# Knot position can't be on the last day (that happens if intervention is on that day).
# In that case, remove it:
if(knot.pos[length(knot.pos)]==length(d1$date)) knot.pos <- knot.pos[-length(knot.pos)]
...
stan_data$f4<-matrix(rep(stan_data$f4,4),ncol=4)
stan_data$f5<-matrix(rep(stan_data$f5,4),ncol=4)
stan_data$f6<-matrix(rep(stan_data$f6,4),ncol=4)
stan_data$f7<-matrix(rep(stan_data$f7,4),ncol=4)
...
gen.stan<-function(nAlpha, stan_filename){
  ...
  int cases[N2,1]; // reported cases
  int deaths[N2, 1]; // reported deaths -- the rows with i > N contain -1 and should be ignored
  int deathsh[N2, M];
  int deathsc[N2, 1];
...
parameters {
real<lower=0> mu[1]; // intercept for Rt
"
txt4<-"
matrix<lower=0>["
txt41<-", 1] alpha_hier; // sudo parameter for the hier term for alpha
"
txt5<-"
real<lower=0> y[1];
real<lower=0> phi_dc;
...
txt61<-"//];"
txt7<-"
real <lower=0> ifr_noise_1[1];
real <lower=0> ifr_noise_2[1];
...
txt81<-", 1] alpha;"
txt9<-"
matrix[N2, 1] prediction = rep_matrix(0,N2,1);
matrix[N2, 1] E_deaths  = rep_matrix(0,N2,1);
matrix[N2, M] E_deathsh  = rep_matrix(0,N2,M);
matrix[N2, 1] E_deathsht  = rep_matrix(0,N2,1);
matrix[N2, 1] E_deathsc  = rep_matrix(0,N2,1);
matrix[N2, 1] E_cases  = rep_matrix(0,N2,1);
...
matrix[N2, 1] Rt = rep_matrix(0,N2,1);
matrix[N2, 1] Rt_adj = Rt;

{
  matrix[N2,1] cumm_sum = rep_matrix(0,N2,1);
  for(m in 1:1){"
  ...
  E_hospitalsi[1, m]= 1e-15 * prediction[1,1];
  for (i in 2:N2){
  for(j in 1:(i-1)){
  E_hospitalsi[i,m] += prediction[j,1] * f3[i-j,m] * ifr_noise_3[m];
  }
  }
  ...
  for (m in 1:M){
  for (i in 1:N2){
  E_deathsht[i,1] +=E_deathsh[i,m];
  }
  }

  for (i in 1:N2){
  E_deaths[i,1] += E_deathsht[i,1]+E_deathsc[i,1];
  }
...
matrix[N2, 1] prediction0 = rep_matrix(0,N2,1);
vector[N2-N] logLikcases;
vector[N2-N] logLikdeathsc;
matrix[N2-N, M] logLikicuout = rep_matrix(0,N2-N,M);
matrix[N2-N, M] logLikicuin = rep_matrix(0,N2-N,M);
matrix[N2-N, M] logLikicu = rep_matrix(0,N2-N,M);
matrix[N2-N, M] logLikhospout=rep_matrix(0,N2-N,M);
matrix[N2-N, M] logLikhospin=rep_matrix(0,N2-N,M);
matrix[N2-N, M] logLikhosp=rep_matrix(0,N2-N,M);
matrix[N2-N, M] logLikdeathsh=rep_matrix(0,N2-N,M);
int<lower=0> h;
{
  matrix[N2,1] cumm_sum0 = rep_matrix(0,N2,1);
  for (m in 1:1){
  for (i in 2:N0){
  ...
  for (i in (N+1):N2){
  h=h+1;
  logLikcases[h]=neg_binomial_2_lpmf(cases[i]|E_cases[i, 1], phi_c);
  logLikdeathsc[h]=neg_binomial_2_lpmf(deathsc[i]|E_deathsc[i, 1], phi_dc);
  for (m in 1:M){
  logLikicuout[h,m]=neg_binomial_2_lpmf(icuout[i, m]|E_icuso[i, m], phi_io);
  logLikicuin[h,m]=neg_binomial_2_lpmf(icuin[i, m]|E_icusi[i, m], phi_ii);
  logLikicu[h,m]=neg_binomial_2_lpmf(icu[i, m]|E_icus[i, m], phi_i);
  logLikhospout[h,m]=neg_binomial_2_lpmf(hospout[i, m]|E_hospitalso[i, m], phi_ho);
  logLikhospin[h,m]=neg_binomial_2_lpmf(hospin[i, m]|E_hospitalsi[i, m], phi_hi);
  logLikhosp[h,m]=neg_binomial_2_lpmf(hosp[i, m]|E_hospitals[i, m], phi_h);
  logLikdeathsh[h,m]=neg_binomial_2_lpmf(deathsh[i, m]|E_deathsh[i, m], phi_dh);
  }
  }
}
}
...
cat(txt1,txt2,txt3,txt4,nAlpha,txt41,txt5,txt6,nAlpha,txt61,txt7,txt8,nAlpha,txt81,txt9,txt10,nAlpha,txt10.1,
    txt11,txt12,txt13,txt14,nAlpha,txt141,txt15,txt16,nAlpha,txt161,txt17,file=paste0("combined_hospitals/stan-models/", stan_filename, ".stan"),sep="")
...
options(mc.cores = 4)
  ...
  m = stan_model(paste0('combined_hospitals/stan-models/',
                        stan_filename,
                        '.stan'))
...
estimated.deathsht = out$E_deathsht #added but not needed probably
hospital.names <- c('UCC Ljubljana', 'UCC Maribor', 'UC Golnik', 'GH Celje')
...
save(fit,prediction,dates,reported_cases,deaths_by_country,deathsh_by_country,deathsc_by_country,hosps_by_country,hospsin_by_country,hospsout_by_country,icus_by_country,icusin_by_country,icusout_by_country,countries,estimated.deaths,estimated.deathsc,estimated.deathsh,estimated.deathsht,estimated.deaths.cf,estimated.cases,estimated.hospitals,estimated.hospitalsi,estimated.hospitalso,estimated.icus,estimated.icusi,estimated.icuso,out,covariates,interventions,fit.spline,forecast,natural.spline,fit.spline,hospital.names,
     hosp.hospitalizirani,hosp.hospitaliziraniin,hosp.hospitaliziraniout,hosp.icu,hosp.icuin,hosp.icuout,hosp.deathsh,
     file=paste0(model_input,'/results/',JOBID,'-stanfit.Rdata'))
